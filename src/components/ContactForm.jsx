...
const ContactForm = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await axios.post(
        `${process.env.NEXT_PUBLIC_BASE_URL}/api/emails/welcome`, // Update the endpoint
        {
          email: form.email, // Match the new structure
          name: form.name,
          message: form.message,
          mailSubject: form.mailSubject || "Contact Us Inquiry", // Set a default subject or add another input for it
        }
      );
      setSubmitted(true);
  ...
  // Update the contact information data as needed
  const data = {
    "contacts": {
      "showrooms": [],
      "hotline": [
      ...
      "address": "Flat# 9B, Level# 09, Main Road, Block# H, Aftabnagar, Badda, Dhaka-1212, Bangladesh"
    }
  };
          ...
          <div className="bg-white border w-full flex flex-col lg:flex-row gap-8">
            ...
            <div className="w-full lg:w-1/2 lg:p-6">
              ...
              <form onSubmit={handleSubmit} className="space-y-4 p-8 lg:mt-12">
                ...
                <button
                  type="submit"
                  className="w-full bg-[#ffcd8c] text-white p-2 rounded-md hover:bg-dcfdfs transition duration-300"
                >
                  Send
                </button>
                {submitted && (
            ...
            <div className="w-full lg:w-1/2 bg-contactBlue text-white p-6 text-sm bg-[#ffcd8c]">
